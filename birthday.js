export const getClosest = (data, now) => {
  let futureBdays = []
  ...
  const closestBdayObj = futureBdays.sort((a, b) => a.birthdate - b.birthdate)[0]

  return closestBdayObj
}

export const getCountdown = (birthdate) => {
...
export const isBdayToday = (client) => {
  const now = new Date()
  const bdaysData = [
    {
      name: "Pattu",
